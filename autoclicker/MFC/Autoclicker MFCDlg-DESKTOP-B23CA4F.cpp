...
#define HOTKEYMSG 0x0410
#define STOPKEYMSG 0x0411
...
static UINT GetKeyBind(LPVOID pParam)
{
	...
	while (true)
	{
		for (int i = 0; i < 176; ++i)
		{
			...
			{
				*Hotkeystr = VKeyList[i].Description;
				PostMessage(MainWindowHWND, 0x0410, reinterpret_cast<WPARAM>(Hotkeystr), 0);
				return 0;
			}
		}
		Sleep(100);
...
void CAutoclickerMFCDlg::OnBnClickedButton1()
{
	...
	{
		CWinThread *pThread1 = AfxBeginThread(StartAutoclick, ClickingInfoptr);
...
void CAutoclickerMFCDlg::OnBnClickedButton3()
{
	...
	if (Bindstart == FALSE)
	{
		...
		Sleep(200);
		Bindstart = TRUE;
		CWinThread *pThread2 = AfxBeginThread(GetKeyBind, static_cast<LPVOID>(CWnd::GetSafeHwnd()));
	}
	else
	{
		Bindstart = FALSE;
